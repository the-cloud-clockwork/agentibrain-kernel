...
| `agentibrain uninstall [--name brain] [--no-unlink] [--dry-run]` | Run `docker compose down` for the detected stack, stop any remaining agentibrain compose project, then unlink the brain profile from AgentiHooks. Configuration, vault data, and volumes survive. |
