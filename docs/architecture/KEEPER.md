...
`ENABLE_SHEETS=true` and `ENABLE_DOCS=true` must be set in your platform's drive-sync deployment. Example: your platform repo's drive-sync Lambda config.

### Drive location

Files land at: **Drive → `misc/brain-keeper/`** — the configured Drive root for the delegate service account, accessible from Drive web, Drive mobile app, or Sheets/Docs mobile apps.
...
- **Scheduled daily test:** add a cron entry in `workflows/crons/` that dispatches `brain-keeper test` daily at 06:00 UTC. Report is ready at the start of the operator's day.
