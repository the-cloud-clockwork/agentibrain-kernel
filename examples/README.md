...
│   ├── brain-api/{values-dev,values-prod}.yaml.example
...
│   └── brain-ops/values.yaml.example          (singleton — one per cluster)
│
└── argocd/                 ← ArgoCD Application CRs (operator-side)
    ├── agentibrain-root.yaml.example           (app-of-apps, picks one env)
    ├── dev/agentibrain-{brain-api,embeddings,brain-keeper,brain-ops,mcp}.yaml.example
    └── prod/agentibrain-{brain-api,embeddings,brain-keeper,mcp}.yaml.example
                                                 (no brain-ops-prod — singleton)
```
...
| `<your-ops-namespace>` | K8s namespace for cluster-singleton workloads like brain-ops (e.g. `brain-ops`) |
...
- **brain-api, embeddings, brain-keeper** — deploy one per env (`-dev`, `-prod`).
- **brain-ops, mcp-agentibrain** — singletons. One CronJob set + one MCP server per cluster, since they operate on a single operator vault.
