...
def _source_checkout() -> bool:
    """True when running out of a git checkout rather than an installed wheel.

    A repo-root egg-info shadows the installed dist-info whenever the CWD is the
    checkout, so direct_url.json alone reports a wheel install that is not there.
    """
...
def install_mode() -> str:
    ...
    """
    if _source_checkout() or is_editable():
        return "editable"
